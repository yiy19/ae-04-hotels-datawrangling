...
The following data describes hotel bookings where the guest is not from the US and the guest has cancelled the booking within one day.
...
The following data shows hotel bookings where the guest involved at least one child or one baby. 
...
It's more likely to find bookings with children or babies in city hotels than in resort hotels. There are 2,091 bookings with more than one child or baby in city hotels whereas there are 1,655 bookings with more than one child or baby in resort hotels.
...
hotels %>% 
  ...
  arrange(desc(n))
```
The most common number of adults booking a hotel is two. A couple of things are interesting. One is that more than twenty people book hotels at occasions. Two is that minors not accompanied by adults also book hotels quite often. 
...
```{r freq-adults-cancellation}
hotels %>% 
  filter(is_canceled == 1) %>% # examine canceled bookings
  count(adults) %>% # count the number of cases for each number of adults 
  arrange(desc(n)) # arrange the table by frequency from greatest to smallest
...
The output above shows that the surprisingly large number of groups booking a hotel together have all been canceled. 
...
City hotels are generally higher than resort hotels in terms of average daily rate. 
...
```{r unusual-arr.date-people}
hotels %>% 
  filter(adr == -6.38 | adr == 5400) %>% 
  select(hotel, arrival_date_year, arrival_date_month, adults, children, babies, adr)

```
The hotel type associated with a negative average daily rate is resort hotel. The hotel type associated with a really high average daily rate is city hotel. 
